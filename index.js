...
if (artists[8].name === 'Vincent Van Dough') artists[8].name = 'Vincent Van Gogh';
...
function lotsOfArt(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].paintings >= 100) bigPainters.push(arr[i].name);
  }
  return bigPainters;
}
...
function artistInfo(arr, name){
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].name === name) return arr[i].bio;
  }
}
...
function artistByCountry(arr, nationality){
  const newArr = [];
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].nationality === nationality) newArr.push(arr[i].name);
  }
  return newArr;
}
